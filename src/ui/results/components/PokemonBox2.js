...
export const PokemonBox = ({
  ...
  const isPokemonSpeciesLoaded = (pokemonSpecies || {}).name !== undefined; // idk
        ...
        <ul>
          <li className="flex items-center mb-4">
            ...
            {pokemon.types.map(({ type }, index) => (
              <div className="mr-2" key={`${pokemon.name}-type-${type.name}`}>
          ...
          {isPokemonSpeciesLoaded ? (
            <>
              <li className="mb-4">
                <p>
                  {getFlavorText(pokemonSpecies.flavor_text_entries, language)}
                </p>
              </li>
              <li>
